...
function showAddModal(type) {
    ...
    switch (type) {
        ...
        case 'aircraft':
            ...
            break;
        case 'airport':
            editMode = { active: false, type: 'airport', id: null };
            showAirportModal();
            break;
        case 'maintenance':
            editMode = { active: false, type: 'maintenance', id: null };
...
async function showFlightModal() {
    ...
    try {
        // Cargar datos para selects incluyendo crew members
        const [airports, aircrafts, pilots, crewMembers] = await Promise.all([
            fetch(`${API_URL}/airports`).then(r => r.json()),
            fetch(`${API_URL}/aircraft`).then(r => r.json()),
            fetch(`${API_URL}/pilots`).then(r => r.json()),
            fetch(`${API_URL}/crew-members`).then(r => r.json())
        ...
        // Si estamos editando, obtener crew members asignados
        let assignedCrewIds = [];
        if (editing && editMode.id) {
            try {
                const assignedCrew = await fetch(`${API_URL}/flights/${editMode.id}/crew`).then(r => r.json());
                assignedCrewIds = assignedCrew.map(c => c.crew_member_id);
            } catch (e) {
                console.warn('Could not load assigned crew:', e);
            }
        }
            ...
            <form id="flight-form">
                ...
                <div class="form-group"><label>Departure Airport</label>
                    <select id="departure_airport_id" required>
                        ...
                        ${airports.map(a => `<option value="${a.airport_id}">${a.city} (${a.iata_code})</option>`).join('')}
                    </select>
                </div>
                ...
                <div class="form-group"><label>Aircraft</label>
                    <select id="aircraft_id" required>
                        ...
                        ${aircrafts.map(a => `<option value="${a.aircraft_id}">${a.tail_number} - ${a.aircraft_model}</option>`).join('')}
                    </select>
                </div>
                <div class="form-group"><label>Pilot Commander</label>
                    <select id="pilot_command_id" required>
                        <option value="">Select Pilot</option>
                        ${pilots.map(p => `<option value="${p.pilot_id}">${p.first_name} ${p.last_name} (${p.current_rank})</option>`).join('')}
                    </select>
                </div>
                ...
                <!-- Sección de Cabin Crew -->
                <div class="form-group">
                    <label>
                        <i class="fas fa-users"></i> Cabin Crew (Optional)
                        <small style="color: var(--muted); font-weight: normal;">Select crew members for this flight</small>
                    </label>
                    <div class="crew-selection">
                        ${crewMembers
                .filter(c => c.employment_status === 'Active')
                .map(c => `
                                <label class="crew-checkbox">
                                    <input type="checkbox" 
                                           name="crew_member" 
                                           value="${c.crew_member_id}"
                                           ${assignedCrewIds.includes(c.crew_member_id) ? 'checked' : ''}>
                                    <span class="crew-info">
                                        <strong>${c.first_name} ${c.last_name}</strong>
                                        <small>${c.current_role}</small>
                                    </span>
                                </label>
                            `).join('')
            }
                        ${crewMembers.filter(c => c.employment_status === 'Active').length === 0
                ? '<p style="color: var(--muted); text-align: center; padding: 20px;">No active crew members available</p>'
                : ''
            }
                    </div>
                </div>
        ...
        console.error('Error loading flight modal:', error);
        showNotification('Error loading flight form', 'error');
    }
}
...
async function loadCrewAssignments() {
    try {
        const resp = await fetch(`${API_URL}/flight-crew-assignments`);
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
        ...
        if (tbody) {
            tbody.innerHTML = assignments.map(a => `
                <tr>
                    ...
                    <td><strong>${safeText(a.flight_number)}</strong></td>
                    <td>${safeText(a.first_name)} ${safeText(a.last_name)}</td>
                    <td><span class="status-badge status-active">${safeText(a.current_role)}</span></td>
                    <td class="action-buttons">
                        <button class="btn btn-danger btn-small" onclick="deleteCrewAssignment(${a.assignment_id})">
                            <i class="fas fa-trash"></i>
                        </button>
                    </td>
                </tr>
        ...
        console.error("Error loading crew assignments:", e);
        showNotification("Error loading crew assignments", "error");
    }
}

async function showAssignCrewModal() {
    document.getElementById('modal-title').textContent = 'Assign Crew to Flight';
    const modalBody = document.getElementById('modal-body');

    try {
        // Cargar vuelos y crew members
        const [flights, crewMembers] = await Promise.all([
            fetch(`${API_URL}/flights`).then(r => r.json()),
            fetch(`${API_URL}/crew-members`).then(r => r.json())
        ]);
        ...
        modalBody.innerHTML = `
            <form id="assign-crew-form">
                ...
                <div class="form-group">
                    ...
                    <select id="assign_crew_member_id" required>
                        <option value="">Select a crew member</option>
                        ${crewMembers
                .filter(c => c.employment_status === 'Active')
                .map(c => `
                                <option value="${c.crew_member_id}">
                                    ${c.first_name} ${c.last_name} - ${c.current_role}
                                </option>
                            `).join('')
            }
                    </select>
        ...
        document.getElementById('assign-crew-form').onsubmit = e => {
            e.preventDefault();
            saveCrewAssignment();
        };
        ...
        console.error('Error loading assign crew modal:', error);
...
async function saveCrewAssignment() {
    try {
        const flightId = parseInt(document.getElementById('assign_flight_id').value);
        const crewMemberId = parseInt(document.getElementById('assign_crew_member_id').value);

        if (!flightId || !crewMemberId) {
            showNotification('Please select both flight and crew member', 'error');
            return;
        }

        const resp = await fetch(`${API_URL}/flights/${flightId}/crew`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (!resp.ok) {
            ...
            throw new Error(error.error || 'Error assigning crew');
        }

        const result = await resp.json();
        showNotification(result.message || 'Crew assigned successfully', 'success');
        ...
        showNotification(e.message || "Error assigning crew", "error");
    }
}

async function deleteCrewAssignment(assignmentId) {
    if (!confirm('Are you sure you want to remove this crew assignment?')) return;

    try {
        const resp = await fetch(`${API_URL}/flight-crew-assignments/${assignmentId}`, {
            method: 'DELETE'
        });
        ...
        showNotification(e.message || "Error deleting assignment", "error");
    }
}